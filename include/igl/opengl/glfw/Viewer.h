...
public:
    ...
    int select_rotate_index = -1;
    std::vector< std::vector<int> > path_point_global;
    int select_point2;
    Eigen::MatrixXd select_point1;
    //求dijkstra算法  存放添加的点的id
    std::vector<int> point_id, point_num;
    //求两点间测地线  存放固定两点的id
    std::vector<int> point2_id;
